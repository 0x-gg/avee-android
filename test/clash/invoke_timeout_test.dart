...
ChangeProxyParams _minimalChangeProxyParams() => const ChangeProxyParams(
      groupName: "GLOBAL",
      proxyName: "DIRECT",
    );
...
void main() {
  ...
  group('mutating calls fail-closed on timeout (the bug fix)', () {
    ...
    test('validateConfig passes a non-null onTimeout returning an error sentinel',
      ...
      expect(result, isNotEmpty,
          reason: 'a timed-out validateConfig must NOT look like "valid" ("")');
      expect(result, startsWith("error:"));
      expect(handler.capturedMethod, ActionMethod.validateConfig);
      expect(handler.capturedOnTimeoutWasNull, isFalse,
          reason: 'validateConfig must wire an explicit onTimeout sentinel');
    });

    test('changeProxy passes a non-null onTimeout returning an error sentinel',
        () async {
      final handler = _CapturingHandler();
      final result = await handler.changeProxy(_minimalChangeProxyParams());
      expect(result, isNotEmpty,
          reason: 'a timed-out changeProxy must NOT look like success ("")');
      expect(result, startsWith("error:"));
      expect(handler.capturedMethod, ActionMethod.changeProxy);
      expect(handler.capturedOnTimeoutWasNull, isFalse,
