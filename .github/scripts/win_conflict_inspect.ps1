...
$dirs = @(
  'C:\Program Files\avee',
  'C:\Program Files\FlClashX',
  'C:\Program Files (x86)\AVEE',
...
Write-Host "-- Uninstall registry entries (AVEE / FlClash) [key | name | ver | InstallLocation] --"
$keys = @(
...
foreach ($k in $keys) {
  Get-ChildItem $k -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    if ($p.DisplayName -match 'AVEE|FlClash') {
...
Get-Service -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'Helper|Clash|AVEE' } | ForEach-Object { Write-Host "  $($_.Name) = $($_.Status)" }
